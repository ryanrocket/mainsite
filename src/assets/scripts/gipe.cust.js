...
let stage;
let console;
const GIPE_DEBUG_MODE = false;
const stages = ['staging', 'testing', 'production', 'preview'];
let GIPE_ERR_INDEX = 0;
const GIPE_XML_REQ = new XMLHttpRequest();
let GIPE_USING_GA = false;
let GIPE_GA_SESSION = 'NA';
let iii; let xxx; let yyy; let lfal; let gipe_cont;
let GIPE_USER_BROWSER;
let GIPE_FULLY_RENDERED = false;
const GIPE_ENVIRONMENT_SETTINGS = {};
let GIPE_POST_KEY_PUBLIC;
const GIPE_POST_DELAY = 2000;
let GIPE_PRE_CONFIRMATION;
const GIPE_PREVENT_PLAYBACK = true;
let GIPE_ERR_OCCURED = false;
...
(function () {
  ...
  (window.dataLayer === null || window.dataLayer === undefined) ? window.dataLayer = [] : (function () { window.dataLayer; GIPE_USING_GA = true; }(window.gipeData === null || window.gipeData === undefined)) ? window.gipeData = {} : window.gipeData = {};
  // vanilla reassure
  if (window.gipeData === undefined) { window.gipeData = {}; }
  if (GIPE_USING_GA) {
    for (iii = 0; iii < window.dataLayer.length; iii++) {
  ...
  function errtag(ind, er, url, line) {
    const d = new Date(); window.gipeData[ind] = {
      MES: er, SCRIPT_URL: url, LINE: line, STAGE: window.releaseStage, TIME: d.toDateString(), GA_SESSION: GIPE_GA_SESSION, BROWSER: GIPE_CAPTURE_BROWSER(), RENDERED: GIPE_FULLY_RENDERED, PAGE: window.location.href,
    };
  }
...
try { $(document).ready(() => { GIPE_FULLY_RENDERED = true; }); } catch (e) {}
...
function GIPE_CAPTURE_BROWSER() {
  if ((navigator.userAgent.indexOf('Opera') || navigator.userAgent.indexOf('OPR')) != -1) { GIPE_USER_BROWSER = ('Opera'); } else if (navigator.userAgent.indexOf('Chrome') != -1) { GIPE_USER_BROWSER = ('Chrome'); } else if (navigator.userAgent.indexOf('Safari') != -1) { GIPE_USER_BROWSER = ('Safari'); } else if (navigator.userAgent.indexOf('Firefox') != -1) { GIPE_USER_BROWSER = ('Firefox'); } else if ((navigator.userAgent.indexOf('MSIE') != -1) || (!!document.documentMode == true)) { GIPE_USER_BROWSER = ('IE'); } else { GIPE_USER_BROWSER = ('unknown'); }
...
function GIPE_REQUEST_POSTER() {
  ...
  if (GIPE_PREVENT_PLAYBACK) {
    const gipe_cors_url = 'https://cors-anywhere.herokuapp.com/https://ryanwans.com/gipe/socket/analytics/input';
    const gipe_pre_url = 'https://ryanwans.com/gipe/socket/analytics/input';
    ...
    if (String(GIPE_PRE_CONFIRMATION.access) === 'allowed' && GIPE_PRE_CONFIRMATION.forward_key === GIPE_POST_KEY_PUBLIC) {
      ...
      GIPE_XML_REQ.setRequestHeader('Content-Type', 'application/json');
      GIPE_XML_REQ.send(JSON.stringify({
        cmd: '!send packet [now]', gipeData: window.gipeData, envSet: GIPE_ENVIRONMENT_SETTINGS, apikey: GIPE_POST_KEY_PUBLIC,
      }));
